...
class RMSNorm(nn.Module):
    ...
    相比 LayerNorm，RMSNorm 不计算均值，只计算方差进行归一化
    """
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        重塑后的 freqs_cis [1, seq_len, 1, ..., 1, head_dim//2]
    """
    ndim = x.ndim
    assert ndim >= 3, "输入张量维度必须至少为3"
    assert freqs_cis.shape[0] == x.shape[1], f"序列长度不匹配: freqs_cis={freqs_cis.shape[0]}, x={x.shape[1]}"
    assert freqs_cis.shape[1] == x.shape[-1] // 2, f"头维度不匹配: freqs_cis期望{x.shape[-1]//2}, 实际{freqs_cis.shape[1]}"

    # 构造广播形状: [1, seq_len, 1, ..., 1, head_dim//2]
    shape = [1] * ndim
